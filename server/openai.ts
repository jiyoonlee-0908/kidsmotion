...
AI는 다음 4개 부분만 작성해주세요. JSON 형식으로 응답하세요:
{
  ...
  "detailedReport": "15년 경력의 아동 운동생리학 박사이자 스포츠 의학 컨설턴트 관점에서 9단계 구조의 상세 종합해설을 작성. 각 섹션은 3-6문장으로 전문적이면서도 부모가 쉽게 이해할 수 있게 작성. 번호 대신 적절한 이모티콘 사용, 중요한 내용은 **굵게** 강조: 🔍 ${data.studentName}의 오늘 한눈에 보기 (측정 결과 핵심 요약과 또래 대비 위치, 전반적인 체력 특성을 3-4문장으로), 🌟 강점 & 잠재력 (뛰어난 항목의 구체적 수치와 백분위, 이를 활용한 스포츠 종목 추천, 장점을 더 발전시킬 방법을 4-5문장으로), 🎯 우선 개선 영역 (가장 약한 부분의 구체적 분석, 방치 시 위험성, 개선 시 기대효과를 4-5문장으로), 📅 이번 주 해야 할 일 (매일 실행 가능한 구체적 운동법, 시간과 강도, 체크포인트를 3-4문장으로), 🎲 이번 달 목표 (구체적 수치 목표, 중간 점검 방법, 달성 가능한 백분위 상승치를 3-4문장으로), 🗺️ 3개월 로드맵 (월별 단계적 계획, 각 단계의 목표 수치와 운동 방법을 4-5문장으로), 👨‍👩‍👧‍👦 부모 참여 운동법 (가족 단위 운동 종류, 격려 방법, 재미있는 게임 요소를 3-4문장으로), ⚠️ 안전 주의사항 (운동 중단 신호, 부상 예방법, 응급상황 대처를 3-4문장으로), 📈 다음 측정 예상 개선치 (3개월 후 예상 수치, 가장 개선될 항목, 목표 달성 조건을 3-4문장으로). 프리미엄 서비스답게 각 섹션을 상세하고 실용적으로 작성"
}
