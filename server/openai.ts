...
AI는 다음 4개 부분만 작성해주세요. JSON 형식으로 응답하세요:
{
  ...
  "detailedReport": "CRITICAL: 반드시 이모티콘으로 시작하세요. # 숫자는 절대 사용하지 마세요. 15년 경력의 아동 운동생리학 박사이자 스포츠 의학 컨설턴트 관점에서 9단계 구조의 상세 종합해설을 프리미엄 서비스 수준으로 작성하세요. 각 섹션은 반드시 4-6문장으로 작성하며, BMI ${data.bmi}와 체중 ${data.weight}kg, 키 ${data.height}cm 관련 분석도 포함하세요. 형식 예시: '🔍 ${data.studentName}의 오늘 한눈에 보기\n${data.studentName}은 또래보다 뛰어난 심폐지구력을 가지고 있으며, 이는 다양한 운동에서 장점이 됩니다. 하지만 파워 지속력이 부족하여 장기적인 운동 능력 향상에 방해가 될 수 있습니다. BMI ${data.bmi} 수준으로 체중이 운동 수행에 유리한 조건을 제공합니다. 전반적으로 균형잡힌 발달을 보이고 있어 긍정적입니다.\n\n🌟 강점 & 잠재력\n${data.studentName}은 심폐지구력에서 상위 XX%를 차지하고 있으며, 이는 또래 아이들 중 상위 XX명 안에 든다는 것입니다. 심폐지구력은 장시간 운동을 유지하는 능력으로, 다양한 스포츠에서 중요한 요소입니다. 이러한 강점을 활용하여 달리기, 자전거 타기와 같은 유산소 운동을 통해 체력을 더욱 쌓을 수 있습니다. 또한, 스프린트 파워가 상위 XX%에 위치하고 있어, 짧은 거리의 빠른 속도 내기에서도 경쟁력을 가집니다. 이러한 능력은 축구나 태권도와 같은 활동에서 유리하게 작용할 수 있습니다.' 이런 식으로 9개 섹션 모두 4-6문장씩 상세하게 작성하세요. 절대 # 숫자 형식 사용 금지, 반드시 이모티콘으로 시작"
}
