...
**절대 필수 지침:**
1. ${data.studentName}의 이름을 부를 때는 반드시 "${data.studentName}님"으로 호칭 (님 필수!)
2. 예시: "김민준님은", "박주혁님의", "이영희님이" 등
3. 절대로 "${data.studentName}은", "${data.studentName}의" 등으로 쓰지 말고 항상 "님" 붙이기
4. 구체적인 백분위 수치와 개선 목표 포함
5. 실용적이고 실행 가능한 운동 처방 제시
6. 강조(**굵게**)와 줄바꿈으로 가독성 향상
`;
