...
{
  "summary": "15년 경력의 아동 운동생리학 박사 관점에서 부모가 이해하기 쉽게 강점과 보완점을 정확히 3문단으로 작성. 각 문단은 정확히 3줄로 구성하고, **강조문구**는 절대 사용하지 마세요. 강조가 필요한 부분은 빨간 글씨로 표시하기 위해 <span style='color: red;'>내용</span> 형식을 사용하세요",
  "balanceComment": "좌우 밸런스 차이 ${data.balanceDifference}%에 대한 분석. 성장과의 관련성을 포함하되 의학적 진단/치료 문구는 피하고, 4-7줄로 상세 작성",
  "comprehensiveAnalysis": "체력 종합분석 AI 해설. BMI와 체중이 운동 수행에 미치는 영향을 포함하여 분석하고, 줄바꿈(\\n)과 강조(**굵게**)를 적절히 사용하여 읽기 쉽게 작성. ${data.studentName}의 이름을 자연스럽게 활용하여 개인 맞춤형 분석 제공. 체중 대비 파워 분석과 성장기 아동의 체격적 특성을 반영한 전문적 해설",
  "detailedReport": "CRITICAL: 반드시 이모티콘으로 시작하세요. # 숫자는 절대 사용하지 마세요. 15년 경력의 아동 운동생리학 박사이자 스포츠 의학 컨설턴트 관점에서 9단계 구조의 상세 종합해설을 프리미엄 서비스 수준으로 작성하세요. 각 섹션은 반드시 5-7문장으로 구체적이고 상세하게 작성하며, 실행 가능한 구체적 방법과 수치를 포함하세요. BMI ${data.bmi}와 체중 ${data.weight}kg, 키 ${data.height}cm 관련 분석도 포함하세요. 백분위를 구체적 순위로 설명하고(예: 상위 XX% = 또래 100명 중 XX등), 운동 방법은 시간/횟수/빈도를 구체적으로 제시하세요. 형식 예시: '🔍 ${data.studentName}의 오늘 한눈에 보기\n${data.studentName}은 또래보다 뛰어난 심폐지구력을 가지고 있으며, 이는 다양한 운동에서 장점이 됩니다. 하지만 파워 지속력이 하위 17%로 또래 100명 중 83등에 해당하여 장기적인 운동 능력 향상에 방해가 될 수 있습니다. BMI ${data.bmi} 수준으로 체중이 운동 수행에 유리한 조건을 제공합니다. 전반적으로 균형잡힌 발달을 보이고 있어 긍정적입니다. 이를 방치하면 운동에 대한 흥미를 잃을 수 있습니다.\n\n🌟 강점 & 잠재력\n${data.studentName}은 심폐지구력에서 상위 63%를 차지하고 있으며, 이는 또래 아이들 중 상위 37명 안에 든다는 것입니다.\n\n💡 우선 개선 영역\n파워 지속력이 하위 17%로 또래 100명 중 83등에 해당합니다.\n\n🏃 이번 주 해야 할 일\n매일 10분간의 간단한 인터벌 달리기로 파워 지속력을 강화할 수 있습니다.\n\n📅 이번 달 목표\n이번 달의 목표는 파워 지속력 백분위를 25%까지 끌어올리는 것입니다.\n\n🗓️ 3개월 로드맵\n1개월차에는 파워 지속력을 25%로 향상시키는 것을 목표로 합니다.\n\n👪 부모 참여 운동법\n가족과 함께 할 수 있는 운동으로는 가족 달리기, 자전거 타기가 있습니다.\n\n⚠️ 안전 주의사항\n운동 중 즉시 중단해야 하는 신호로는 심한 가슴 통증, 호흡곤란이 있습니다.\n\n📈 다음 측정 예상 개선치\n3개월 후, ${data.studentName}의 파워 지속력은 30%에 도달하여 약 13%의 개선을 기대할 수 있습니다.' 이런 식으로 9개 섹션 모두 5-7문장씩 매우 상세하게 작성하세요. 절대 # 숫자 형식 사용 금지, 반드시 이모티콘으로 시작"
}

**중요 지침:**
1. 체력요약카드는 반드시 3문단 9줄로 상세하게 작성 - 절대 짧게 쓰지 말고 풍부한 내용으로
2. 좌우밸런스 AI 코멘트: 성장 관련성 포함, 의료법 준수, 4-7줄로 상세하게
3. 체력종합분석 AI 종합해설: 가독성 최우선, 줄바꿈과 강조 필수, 상세한 분석
4. 모든 섹션은 상세하고 구체적으로 작성하되 간결함을 추구하지 말고 충분한 설명 제공
`;
