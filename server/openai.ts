...
AI는 다음 4개 부분만 작성해주세요. JSON 형식으로 응답하세요:
{
  ...
  "detailedReport": "9단계 구조의 상세 종합해설을 마크다운 형식으로 작성. 각 섹션은 최소 4-6문장으로 구체적이고 전문적으로 작성: # 1. ${data.studentName}의 오늘 한눈에 보기 (측정 결과 핵심 요약과 또래 대비 위치, 전반적인 체력 특성), # 2. 강점 & 잠재력 (뛰어난 항목의 구체적 수치와 백분위, 이를 활용한 스포츠 종목 추천, 장점을 더 발전시킬 방법), # 3. 우선 개선 영역 (가장 약한 부분의 구체적 분석, 방치 시 위험성, 개선 시 기대효과), # 4. 이번 주 해야 할 일 (매일 실행 가능한 구체적 운동법, 시간과 강도, 체크포인트), # 5. 이번 달 목표 (구체적 수치 목표, 중간 점검 방법, 달성 가능한 백분위 상승치), # 6. 3개월 로드맵 (월별 단계적 계획, 각 단계의 목표 수치와 운동 방법), # 7. 부모 참여 운동법 (가족 단위 운동 종류, 격려 방법, 재미있는 게임 요소), # 8. 안전 주의사항 (운동 중단 신호, 부상 예방법, 응급상황 대처), # 9. 다음 측정 예상 개선치 (3개월 후 예상 수치, 가장 개선될 항목, 목표 달성 조건). 프리미엄 서비스답게 각 섹션을 길고 상세하게 작성"
}
