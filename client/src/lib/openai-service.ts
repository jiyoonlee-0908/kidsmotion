...
export async function generateFitnessAnalysis(data: FitnessAnalysisRequest): Promise<AIAnalysisResponse> {
  try {
    const response = await apiRequest("POST", "/api/analysis/generate", data);
    return await response.json();
  } catch (error) {
    console.error("Failed to generate fitness analysis:", error);
    ...
    return {
      ...
      overallAssessment: `${data.studentName}은(는) 전체적으로 ${data.overallPercentile}%의 체력 수준을 보이며, 꾸준한 노력을 통해 더 큰 발전이 가능합니다.`
    };
  }
}
