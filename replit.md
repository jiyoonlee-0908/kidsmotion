...
- July 9, 2025: Supabase 데이터 저장 시스템 완전 구축 및 키/몸무게 매핑 완료
  - 웹앱에서 Supabase로 실시간 데이터 저장 기능 추가
  - 참가자 정보, 테스트 세션, 파워 데이터, 분석 결과 자동 저장
  - 이름 기반 자동 검색 및 폼 채우기 기능 검증 완료
  - 키/몸무게 데이터 자동 매핑: participants 테이블에서 직접 가져오기 구현
  - 박시아 실제 데이터로 저장/불러오기 테스트 성공 (키: 110cm, 몸무게: 20kg)
